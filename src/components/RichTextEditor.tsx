...
import {
  ...
  ImagePlus, Link as LinkIcon, Undo, Redo, Highlighter,
  Loader2, Type,
} from 'lucide-react';
...
export default function RichTextEditor({ content, onChange, error }: RichTextEditorProps) {
  ...
  const editor = useEditor({
    extensions: [
      ...
      Placeholder.configure({
        placeholder: 'Write your content here... Use the toolbar to format text, add images, and more.',
      }),
      Link.configure({
        openOnClick: false,
        autolink: true,
    ...
    editorProps: {
      attributes: {
        class: 'prose prose-slate prose-sm max-w-none focus:outline-none min-h-[320px] px-4 py-3',
  ...
  const getActiveStateText = (isActive: boolean, actionName: string): string => {
    return isActive ? `${actionName} active` : `${actionName} inactive`;
  };

  const ToolbarButton = ({
    onClick, isActive = false, disabled = false, children, title, ariaLabel,
  }: {
    onClick: () => void;
    isActive?: boolean;
    disabled?: boolean;
    ...
    <button
      ...
      className={`p-1.5 rounded-lg text-slate-500 hover:text-slate-900 hover:bg-slate-100 transition-colors disabled:opacity-40 disabled:cursor-not-allowed ${
        isActive ? 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100 hover:text-indigo-700' : ''
      }`}
    >
      {children}
    </button>
  ...
  const Divider = () => <div className="w-px h-5 bg-slate-200 mx-1" aria-hidden="true" />;
    ...
    <div className="w-full">
      ...
      >
        ...
        <div className="flex items-center gap-1 px-1">
          ...
          <label htmlFor="font-size-select" className="sr-only">Font size (1-50 pixels)</label>
          <input
            id="font-size-select"
            type="number"
            min="1"
          ...
          />
          <span className="text-xs text-slate-500" aria-hidden="true">px</span>
        </div>
        ...
        <ToolbarButton 
          ...
          ariaLabel={uploading ? 'Uploading image, please wait' : 'Upload image from computer'}
        >
