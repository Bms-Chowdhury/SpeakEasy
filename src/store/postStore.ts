...
import { Post, CreatePostData, UpdatePostData, PostCategory, PostType, Level } from '../lib/types';
...
export const usePostStore = create<PostState>((set, get) => ({
  ...
  getPostsByType: (type: PostType) => {
    return get().posts.filter(p => p.type === type && p.status === 'published');
  },

  getPostsByCategory: (category: PostCategory) => {
    return get().posts.filter(p => p.category === category && p.status === 'published');
  },

  getPostsByLevel: (level: Level) => {
    return get().posts.filter(p => p.level === level && p.status === 'published');
  },

  getTrendingPosts: () => {
    const trendingSlugs = ['how-to-speak-english-fast', 'grammar-mistakes-advanced-learners-make', 'essential-english-phrases-for-travelers'];
    return get().posts.filter(p => trendingSlugs.includes(p.slug) && p.status === 'published');
  },

  getRelatedPosts: (post: Post, limit = 3) => {
    return get()
      .posts.filter(p => p.id !== post.id && p.status === 'published' && (p.category === post.category || p.level === post.level))
      .slice(0, limit);
  },
